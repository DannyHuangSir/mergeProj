...
public class MyStringUtil {
	...
	public static String nullToString(Object inputObj) {
		...
		try {
			if (inputObj != null) {
				ret = inputObj.toString().trim().replace("　", "");
			}
		} catch (Exception e) {
			// do nothing.
		}
	...
	public static String maskStr(String str, int startIndex, int length) {
		String maskedStr = "";
		if(str != null) {
			String mask = "";
			...
			maskedStr = str.substring(0, startIndex) + mask;
			if(startIndex + length < str.length()) {
				maskedStr += str.substring(startIndex + length);
			}
