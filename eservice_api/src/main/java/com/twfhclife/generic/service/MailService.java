...
import com.twfhclife.generic.model.MyAuthenticatorBean;
...
//		ssl = "false";
	...
	public void sendMail(String content, String subject, List<String> mailTo, List<String> mailCc, List<File> listFile)
		...
		//test-code-start
//		Properties properties = new Properties();
//		properties.put("mail.smtp.sender", "mailsender");
...
//		properties.put("mail.smtp.host", "mail.smtp.host");
		...
		if ("10.7.160.12".equals(properties.get("mail.smtp.host"))) {
			logger.debug("********臺銀人壽環境********");
...
//		String mailTos[] = mailTo.split(";");// message設置接收人
		...
		for(String mail : mailTo) {
			logger.info("send email,mail="+mail);
	...
	public static void main(String[] agrs) throws AddressException, MessagingException {
		...
		try {
			...
			List<String> mailTos = Arrays.asList("david.yu03@gmail.com","david.yu04@gmail.com");
			mail.sendMail("此為測試信件內容!!!!", "測試信件主旨!!!", mailTos, null, null);
		} catch (Exception e) {
