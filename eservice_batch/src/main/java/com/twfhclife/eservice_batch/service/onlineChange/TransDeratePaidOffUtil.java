...
public class TransDeratePaidOffUtil {
	private static final Logger logger = LogManager.getLogger(TransDeratePaidOffUtil.class);
	private static final String TRANS_TYPE = "DERATE_PAID_OFF";
	private static final String TRANS_STATUS = "8"; // 1:已接收
			...
			for(TransVo transVos : transList) {
				...
				if(transPolicyVo != null) {
					...
					if(StringUtils.isNotEmpty(nextTredPaabDate)) {
						...
						// 介接代碼 => 008 生效日(用繳別及已轉應繳日,計算下一期應繳日 為 TransDeratePaidOff 的nextTredPaabDate 欄位
