...
public class BatchReport01Service {

	private static final Logger logger = LogManager.getLogger(BatchReport01Service.class);
	...
	/**
	 * 處理主要程式流程 for Report01 - 帳號登入統計報表
	 */
	public void excute(String reportJobId) {
		...
		try {
			...
			String conditionAllCht = paramDao.getParameterValueByCode(SYSTEM_ID, "REPORT_01_CONDITION_CHT_FORMAT");
			String targetPath = paramDao.getParameterValueByCode(SYSTEM_ID, "REPORT_FILE_PATH");
			String filename = paramDao.getParameterValueByCode(SYSTEM_ID, "REPORT_01_FILE_NAME") + "_" 
					+ DateTimeFormatter.ofPattern("yyyyMMddhhmmss").format(startDate) + ".pdf";
			String tableTitle = paramDao.getParameterValueByCode(SYSTEM_ID, "REPORT_01_HEADER_CHT");
			String tableTitleKey = paramDao.getParameterValueByCode(SYSTEM_ID, "REPORT_01_HEADER");
			...
			List<Map<String, Object>> listData = dao.queryReport01(REALM_ID, runtimeJob.getDateRange(), username, roleId, deptId, titleId);
			...
			reportUtil.txt(String.format(conditionAllCht, 
					...
					MyStringUtil.objToStr(username), 
					runtimeJob.getDateRange().toString())).newLine();
