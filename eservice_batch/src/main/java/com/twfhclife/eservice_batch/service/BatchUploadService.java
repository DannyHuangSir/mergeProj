...
public class BatchUploadService {
	...
	public void uploadOnlineChangeFile() {
		...
		String activeTwDate = getActiveTwDate();
		...
		try {
			...
			processList.addAll(new TransPaymodeUtil().appendApplyItems(txtSb, systemTwDate, activeTwDate));        // 繳別:001 | 035
			...
			processList.addAll(new TransInvestmentUtil().appendApplyItems(txtSb, systemTwDate, activeTwDate));// 未來保費投資標的與分配比例:029
...
//				String remoteApplyFilePath = todayApplyFilepPath.replace(localUploadPath, remoteUploadPath);
	...
	private String getActiveTwDate() {
